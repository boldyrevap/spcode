...
/* Animation config */ var renderNodenum = location.hash != "" ? parseInt(location.hash.replace("#", "")) : 25;
var renderNodes = []; // Array for generated nodes
var renderFrames = 60; // Animation Frames per second (FPS)
/* Get the canvas object and upscale it to the complete window size */ var canvasElement = document.getElementById("spectrum");
 ...
 */ function animationInit() {
    clearCanvasElement();
    for(var i = 0; i < renderNodenum; i++)renderNodes.push(new node());
}
/**
 ...
 */ function renderNodespectrum() {
    /* Clear canvas */ clearCanvasElement();
    /* Move every node */ for(var i = 0; i < renderNodes.length; i++){
        /* Get node from array */ var node = renderNodes[i];
        /* Change the node's position */ node.px += node.sx;
        node.py += node.sy;
        /* Check if the node hasn't reached the canvas' border */ if (node.px > canvasLengthX || node.px < 0) node.sx = -node.sx;
        if (node.py > canvasLengthY || node.py < 0) node.sy = -node.sy;
        /* Redraw node */ node.draw();
    }
    /* Link nodes together */ for(var i = 0; i < renderNodes.length; i++){
        /* Get node from array */ var node = renderNodes[i];
        /* Get all the other nodes */ for(var j = 0; j < renderNodes.length; j++){
            /* Get node from array and calculate difference to other node */ var nodes = renderNodes[j];
            var diffX = node.px - nodes.px;
            var diffY = node.py - nodes.py;
            var diffT = Math.sqrt(diffX * diffX + diffY * diffY);
            /* Draw line if total difference between the nodes isn't to big */ if (diffT < canvasLengthX / 4) {
...
/* Initialize and start animation */ animationInit();
animationLoop();

},{}]},["lKzq4","bNKaB"], "bNKaB", "parcelRequired06d")
